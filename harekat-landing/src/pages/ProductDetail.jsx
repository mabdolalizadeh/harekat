...
export default function ProductDetail() {
    ...
    return <MainLayout title={course.name}><TopBarLayout /><Box className="w-full max-w-5xl gap-8 pb-20 pt-28 sm:pt-36">
        ...
        <div className="grid w-full gap-8 md:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
            <div className="overflow-hidden rounded-[var(--radius-2xl)] border border-[var(--border)] bg-card"><img src={course.image} alt={course.name} className="aspect-square w-full object-cover" /></div>
            <div className="flex flex-col gap-4 rounded-[var(--radius-2xl)] border border-[var(--border)] bg-card p-6"><H1 className="text-2xl sm:text-3xl">{course.name}</H1><P className="text-muted">{course.description}</P><div className="mt-auto flex flex-col gap-2">{discounted && <span className="text-sm text-muted line-through">{price(course.price)}</span>}<span className="text-xl font-bold">{price(discounted ? course.salePrice : course.price)}</span><PrimaryButton onClick={add}>{added ? 'به سبد اضافه شد' : 'افزودن به سبد خرید'}</PrimaryButton></div></div>
        </div>
        {course.longDescription && <div className="w-full rounded-[var(--radius-2xl)] border border-[var(--border)] bg-card p-6 sm:p-8"><H2 className="mb-4 text-xl">توضیحات دوره</H2><ReactMarkdown remarkPlugins={[remarkGfm]}>{course.longDescription}</ReactMarkdown></div>}
    </Box></MainLayout>;
}
